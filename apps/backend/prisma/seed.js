...
async function main() {
    ...
    await prisma.user.upsert({ where: { username: 'admin' }, update: {}, create: { username: 'admin', passwordHash, fullName: 'Administrador Paulos', role: 'ADMIN' } });
    await prisma.user.upsert({ where: { username: 'supervisor' }, update: {}, create: { username: 'supervisor', passwordHash, fullName: 'Supervisor General', role: 'SUPERVISOR' } });
    await prisma.user.upsert({ where: { username: 'cajero1' }, update: {}, create: { username: 'cajero1', passwordHash: cashierHash, fullName: 'María García', role: 'CASHIER' } });
    await prisma.user.upsert({ where: { username: 'cajero2' }, update: {}, create: { username: 'cajero2', passwordHash: cashierHash, fullName: 'Juan Pérez', role: 'CASHIER' } });
    ...
    for (const product of products) {
        await prisma.product.upsert({
            ...
            update: {},
            create: { ...product, unit: product.unit || 'UNIT', isFavorite: product.isFavorite || false },
        });
    }
    ...
    console.log('   cajero1 / cajero123 (Cajero - María García)');
    console.log('   cajero2 / cajero123 (Cajero - Juan Pérez)');
}
main()
