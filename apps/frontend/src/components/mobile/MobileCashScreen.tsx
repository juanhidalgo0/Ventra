...
import { Wallet, Lock, AlertTriangle, ChevronRight, CheckCircle2, History } from 'lucide-react';
...
/** Cajas y cierres en el celular: cajas abiertas con su efectivo, cierre con arqueo e historial. */
export default function MobileCashScreen() {
  ...
  const load = useCallback(async () => {
    const [a, p, h] = await Promise.all([
      ...
      api.get('/cash/history', { params: { limit: 30 } }).then((r) => r.data || []).catch(() => []),
    ]);
    setActive(a);
    setPending(p);
    setHistory(h.filter((s: any) => s.closingAmountCounted !== null));
  }, []);
              ...
              <section>
                ...
                <div className="bg-white rounded-2xl border border-slate-200/80 divide-y divide-slate-100 overflow-hidden">
                  {history.map((s) => {
                    ...
                    return (
                      <button key={s.id} onClick={() => setHistoryItem(s)} className="w-full flex items-center gap-3 px-4 py-3 text-left active:bg-slate-50">
                        <span className={`w-9 h-9 rounded-xl flex items-center justify-center shrink-0 ${ok ? 'bg-emerald-50' : 'bg-red-50'}`}>
                          {ok ? <CheckCircle2 className="w-[18px] h-[18px] text-emerald-600" /> : <History className="w-[18px] h-[18px] text-red-600" />}
                        </span>
                        <span className="flex-1 min-w-0">
                          <span className="block text-[14px] font-medium text-slate-800 truncate">{s.terminalName} · {s.user?.fullName}</span>
                          <span className="block text-[12px] text-slate-500">{s.closedAt ? dayTime(s.closedAt) : ''}</span>
                        </span>
                        <span className="text-right shrink-0">
                          <span className="block text-[14px] font-semibold text-slate-900 tabular-nums">{money(s.closingAmountCounted)}</span>
...
function HistorySheet({ session: s, onClose }: { session: any | null; onClose: () => void }) {
  const m = s ? movementTotals(s) : { income: 0, expense: 0, withdrawal: 0 };
  const diff = s ? (s.difference ?? (s.closingAmountCounted - s.closingAmountExpected)) : 0;
  return (
    <Sheet open={!!s} onClose={onClose} title={s ? `${s.terminalName} · ${s.user?.fullName || ''}` : ''}>
      {s && (
        <div className="space-y-4 pt-1">
          <p className="text-[13px] text-slate-500">Abierta {dayTime(s.openedAt)}{s.closedAt ? ` · cerrada ${dayTime(s.closedAt)}` : ''}</p>
          <div className="rounded-2xl border border-slate-200 px-4 py-2 divide-y divide-slate-100">
            <Row label="Debía haber" value={money(s.closingAmountExpected)} />
            <Row label="Se contó" value={money(s.closingAmountCounted)} />
            <Row
              label="Diferencia"
              value={Math.abs(diff) < 1 ? 'Sin diferencia' : `${diff > 0 ? '+' : '−'}${money(Math.abs(diff))}`}
              tone={Math.abs(diff) < 1 ? 'text-emerald-600' : diff > 0 ? 'text-amber-600' : 'text-red-600'}
            />
            {m.expense > 0 && <Row label="Gastos del turno" value={money(m.expense)} />}
            {m.withdrawal > 0 && <Row label="Retiros del turno" value={money(m.withdrawal)} />}
          </div>
          {s.closingNotes && <p className="rounded-2xl bg-slate-50 px-4 py-3 text-[13.5px] text-slate-700">{s.closingNotes}</p>}
        </div>
      )}
    </Sheet>
  );
