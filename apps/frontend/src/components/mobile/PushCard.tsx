...
import { useOrdersVisible } from '../../services/onlineStoreOrders';
...
export default function PushCard() {
  ...
  const hasStore = useOrdersVisible();
  ...
  if (!hasStore || state === 'unsupported' || state === 'granted') return null;
  if (state === 'denied') {
    return (
      <div className="w-full rounded-2xl bg-white border border-slate-200/80 p-4 text-[12.5px] text-slate-500">
        Los avisos están bloqueados. Para recibir los pedidos nuevos, activá las notificaciones de Ventra en los ajustes del celular.
      </div>
    );
  }
  return (
    <button
      ...
      onClick={async () => {
        ...
        try {
          ...
          setState(s);
          if (s === 'granted') toast.success('Listo: te vamos a avisar de cada pedido nuevo');
        } catch (e: any) {
    ...
    >
      ...
      <span className="flex-1 min-w-0">
        <span className="block text-[15px] font-semibold">Avisarme de los pedidos</span>
        <span className="block text-[12px] text-rose-100">Te llega una notificación al celular con cada pedido online, aunque la app esté cerrada.</span>
      </span>
