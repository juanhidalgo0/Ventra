...
export default function SettingsScreen() {
  const { user, resetAdminUnlock } = useAuthStore();
  ...
  const [activeTab, setActiveTab] = useState<'general' | 'posnets' | 'personal' | 'backups' | 'mantenimiento'>('general');
  ...
  const handleDownloadBackup = async (filename: string) => {
    try {
      // Usar window.open o simular una descarga
      const response = await api.get(`/system/backup/download?filename=${filename}`, { responseType: 'blob' });
  ...
  const handleRestoreBackup = async () => {
    ...
    try {
      ...
      toast('Recargando aplicación en 2 segundos para aplicar cambios...', { icon: '🔄' });
      setTimeout(() => {
  ...
  if (isLoading) return <div className="h-full card flex items-center justify-center text-slate-400">Cargando configuración...</div>;

  return (
    <div className="h-full flex flex-col bg-[#f8fafc] overflow-hidden">
      {/* Header */}
      <div className="shrink-0 p-6 pb-4 border-b border-slate-200 bg-white">
        <h1 className="text-2xl font-bold text-slate-800 tracking-tight flex items-center gap-2">
          <Settings className="w-7 h-7 text-indigo-500" /> Configuración General
        </h1>
        <p className="text-[11px] font-bold text-slate-400 uppercase tracking-[0.3em] mt-1">Panel administrativo del maxikiosco</p>
      </div>

      {/* Main Layout Grid */}
      <div className="flex-1 flex flex-col md:flex-row overflow-hidden">
        {/* Settings Navigation Sidebar */}
        <div className="w-full md:w-64 border-b md:border-b-0 md:border-r border-slate-200 bg-white flex flex-row md:flex-col p-3 md:p-4 gap-1.5 shrink-0 overflow-x-auto md:overflow-y-auto custom-scrollbar whitespace-nowrap scrollbar-hide select-none">
          <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest px-3 mb-2 hidden md:block">Grupos de Ajustes</span>
          ...
          <button
            ...
            className={`flex items-center gap-2 md:gap-3 px-3.5 py-2.5 md:px-4 md:py-3 rounded-xl text-[10px] md:text-xs font-bold uppercase tracking-wider transition-all cursor-pointer shrink-0 ${
              activeTab === 'general' ? 'bg-indigo-50 text-indigo-600 shadow-sm' : 'text-slate-500 hover:bg-slate-50 hover:text-slate-800'
            }`}
          >
          ...
          <button
            ...
            className={`flex items-center gap-2 md:gap-3 px-3.5 py-2.5 md:px-4 md:py-3 rounded-xl text-[10px] md:text-xs font-bold uppercase tracking-wider transition-all cursor-pointer shrink-0 ${
              activeTab === 'posnets' ? 'bg-indigo-50 text-indigo-600 shadow-sm' : 'text-slate-500 hover:bg-slate-50 hover:text-slate-800'
            }`}
          >
          ...
          {isAdmin && (
            <button
              ...
              className={`flex items-center gap-2 md:gap-3 px-3.5 py-2.5 md:px-4 md:py-3 rounded-xl text-[10px] md:text-xs font-bold uppercase tracking-wider transition-all cursor-pointer shrink-0 ${
                activeTab === 'personal' ? 'bg-indigo-50 text-indigo-600 shadow-sm' : 'text-slate-500 hover:bg-slate-50 hover:text-slate-800'
              }`}
            >
          ...
          {isAdmin && (
            <button
              ...
              className={`flex items-center gap-2 md:gap-3 px-3.5 py-2.5 md:px-4 md:py-3 rounded-xl text-[10px] md:text-xs font-bold uppercase tracking-wider transition-all cursor-pointer shrink-0 ${
                activeTab === 'backups' ? 'bg-indigo-50 text-indigo-600 shadow-sm' : 'text-slate-500 hover:bg-slate-50 hover:text-slate-800'
              }`}
            >
          ...
          <button
            ...
            className={`flex items-center gap-2 md:gap-3 px-3.5 py-2.5 md:px-4 md:py-3 rounded-xl text-[10px] md:text-xs font-bold uppercase tracking-wider transition-all cursor-pointer shrink-0 ${
              activeTab === 'mantenimiento' ? 'bg-rose-50 text-rose-600 shadow-sm' : 'text-slate-500 hover:bg-rose-50 hover:text-rose-800'
            }`}
          >
          ...
          <AnimatePresence mode="wait">
            {activeTab === 'general' && (
              ...
              >
                ...
                <div className="card bg-white p-6 rounded-2xl border border-slate-150 shadow-sm space-y-4">
                  ...
                  <div className="p-3.5 rounded-xl bg-slate-50 border border-slate-200 text-xs font-semibold text-slate-500 flex flex-col gap-1 shadow-inner">
                    <span className="text-[9px] font-bold text-slate-400 uppercase tracking-wider">Identificador Único (PC UUID)</span>
                    <span className="font-mono text-slate-750 select-all truncate">{terminalUuid}</span>
                  </div>

                  <div className="space-y-1.5">
                    <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider">Nombre del Terminal en Red</label>
                    <div className="flex gap-2">
                      <input 
                        type="text" 
                        value={terminalName} 
                        onChange={(e) => setTerminalName(e.target.value)} 
                        className="flex-1 bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-xs font-bold text-slate-700 outline-none focus:bg-white focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition-all shadow-inner" 
                ...
                <div className="card bg-white p-6 rounded-2xl border border-slate-155 shadow-sm space-y-4">
                  ...
                  <div className="space-y-4">
                    <div className="space-y-1.5">
                      <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider">Monto Inicial de Caja por Defecto ($)</label>
                      <input 
                        type="number" 
                    ...
                    <div className="space-y-3.5">
                      <label className="flex items-center justify-between cursor-pointer group">
                        <div className="max-w-[80%]">
                          ...
                          <span className="text-[9.5px] text-slate-400 block font-medium leading-tight">Permite facturar productos aunque no tengan stock disponible en el sistema.</span>
                        </div>
                        <input 
                          type="checkbox" 
                          checked={allowNegativeStock} 
                          onChange={(e) => setAllowNegativeStock(e.target.checked)} 
                          className="rounded-md border-slate-200 text-emerald-600 focus:ring-emerald-500 h-4.5 w-4.5 cursor-pointer"
                        />
                      </label>
                      ...
                      <label className="flex items-center justify-between cursor-pointer group">
                        <div className="max-w-[80%]">
                          ...
                          <span className="text-[9.5px] text-slate-400 block font-medium leading-tight">Activa notificaciones flotantes en el POS al facturar productos bajo el mínimo configurado.</span>
                        </div>
                        <input 
                          type="checkbox" 
                      ...
                      <label className="flex items-center justify-between cursor-pointer group pt-2 border-t border-slate-100/50">
                        <div className="max-w-[80%]">
                          <span className="text-xs font-bold text-rose-600 block flex items-center gap-1.5">⚡ Modo Rendimiento (Bajo Consumo)</span>
                          <span className="text-[9.5px] text-slate-400 block font-medium leading-tight">Desactiva transiciones, desenfoques de modales y efectos visuales pesados. Recomendado para PCs antiguas.</span>
                        </div>
                        <input 
                          type="checkbox" 
                    ...
                    <div className="space-y-1.5">
                      <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider">Valor de la Hora Trabajada ($)</label>
                      <input 
                        type="number" 
                        value={hourlyRate} 
            ...
            {activeTab === 'posnets' && (
              ...
              >
                <div className="card bg-white p-6 rounded-2xl border border-slate-150 shadow-sm space-y-4">
                  ...
                  <div className="space-y-1.5">
                    <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider">Nuevo Posnet (Nombre)</label>
                    <div className="flex gap-2">
                      <input 
                        type="text" 
                        value={newPosnetName} 
                        onChange={(e) => setNewPosnetName(e.target.value)} 
                        className="flex-1 bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-xs font-bold text-slate-750 outline-none focus:bg-white focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition-all shadow-inner" 
            ...
            {activeTab === 'personal' && isAdmin && (
              ...
              >
                            ...
                            <tr key={u.id} className="border-b border-slate-50 last:border-0 hover:bg-slate-50/30 transition-colors font-medium">
                              ...
                              <td className="py-3 px-4 text-center flex items-center justify-center gap-1">
                                <button 
                                  ...
                                  className="p-2 hover:bg-indigo-50 rounded-lg text-slate-400 hover:text-indigo-600 active:scale-95 transition-all cursor-pointer"
                                  title="Editar cajero"
                                >
                                ...
                                <button 
                                  ...
                                  className="p-2 hover:bg-rose-50 rounded-lg text-slate-400 hover:text-rose-500 active:scale-95 transition-all cursor-pointer disabled:opacity-30"
                ...
                <div className="card bg-white p-6 rounded-2xl border border-slate-150 shadow-sm space-y-4">
                  <h3 className="text-xs font-bold text-slate-750 uppercase tracking-wider flex items-center gap-2 pb-1 border-b border-slate-100">
                ...
                <div className="card bg-white p-6 rounded-2xl border border-slate-150 shadow-sm space-y-5">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-start">
                      ...
                      <div className="space-y-3 pt-2">
                        <label className="flex items-center justify-between cursor-pointer group">
                          ...
                          <input 
                            ...
                            className="rounded-md border-slate-200 text-indigo-600 focus:ring-indigo-500 h-4.5 w-4.5 cursor-pointer"
                          />
                        </label>

                        {autoBackupEnabled && (
                          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} className="space-y-1.5">
                            <label className="block text-[9px] font-bold text-slate-400 uppercase tracking-wider">Hora del Backup</label>
                            <input 
                              type="time" 
                              value={backupTime} 
                              onChange={(e) => setBackupTime(e.target.value)} 
                              className="w-full bg-white border border-slate-200 rounded-xl px-3.5 py-2 text-xs font-bold text-slate-700 outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition-all shadow-inner cursor-pointer" 
                            />
                          </motion.div>
                        )}
                    ...
                    <div className="md:col-span-2 space-y-3">
                      <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">Últimos Backups Guardados en Disco</span>
                      
                      {backups.length === 0 ? (
                        <div className="text-center py-10 bg-slate-50/50 rounded-xl border border-dashed border-slate-200 text-slate-400 text-xs font-bold uppercase tracking-wider">
                        ...
                        <div className="overflow-y-auto max-h-[300px] rounded-xl border border-slate-105 bg-white custom-scrollbar">
                          <table className="w-full min-w-[550px] text-left text-xs border-collapse">
                            <thead>
                              <tr className="bg-slate-50 border-b border-slate-100 text-[9px] uppercase text-slate-400 font-bold tracking-wider">
                ...
                <div className="card bg-white p-6 rounded-2xl border border-rose-100 shadow-sm space-y-4">
                  ...
                  <div className="space-y-4 pt-2 divide-y divide-slate-100">
                    ...
                    <div className="space-y-3 pb-4">
                      ...
                      <AnimatePresence>
                        {showResetStock && (
                          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="p-4 rounded-xl bg-slate-50 border border-slate-200 space-y-3 overflow-hidden">
                            ...
                            <div className="flex gap-2">
                              <input 
                                ...
                                className="flex-1 bg-white border border-slate-200 rounded-lg px-3 py-2 text-xs font-bold text-slate-755 outline-none focus:border-amber-450"
                                placeholder="Escribí STOCK..."
                              />
                              <button 
                                onClick={handleResetStock}
                                disabled={isResettingStock || stockConfirmWord.toUpperCase() !== 'STOCK'}
                                className="px-4 py-2 bg-amber-500 hover:bg-amber-600 disabled:bg-slate-200 disabled:text-slate-400 disabled:cursor-not-allowed text-white rounded-lg text-xs font-bold uppercase tracking-wider active:scale-95 transition-all cursor-pointer"
                              >
                                {isResettingStock ? '...' : 'Confirmar'}
                              </button>
                              <button onClick={() => { setShowResetStock(false); setStockConfirmWord(''); }} className="px-3 py-2 border border-slate-200 bg-white text-slate-500 rounded-lg text-xs font-bold cursor-pointer">Cancelar</button>
                            </div>
                          </motion.div>
                        )}
                      </AnimatePresence>
                          ...
                          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="p-4 rounded-xl bg-slate-50 border border-slate-200 space-y-3 overflow-hidden">
                            ...
                            <div className="flex gap-2">
                              <input 
                                ...
                                className="flex-1 bg-white border border-slate-200 rounded-lg px-3 py-2 text-xs font-bold text-slate-755 outline-none focus:border-rose-455"
                                placeholder="Escribí CAJAS..."
                              />
                              <button 
                                onClick={handleResetCajas}
                                disabled={isResettingCajas || cajasConfirmWord.toUpperCase() !== 'CAJAS'}
                                className="px-4 py-2 bg-rose-500 hover:bg-rose-600 disabled:bg-slate-200 disabled:text-slate-400 disabled:cursor-not-allowed text-white rounded-lg text-xs font-bold uppercase tracking-wider active:scale-95 transition-all cursor-pointer"
                              >
                                {isResettingCajas ? '...' : 'Confirmar'}
                              </button>
                              <button onClick={() => { setShowResetCajas(false); setCajasConfirmWord(''); }} className="px-3 py-2 border border-slate-200 bg-white text-slate-500 rounded-lg text-xs font-bold cursor-pointer">Cancelar</button>
                            </div>
                          </motion.div>
                        )}
                      </AnimatePresence>
                          ...
                          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="p-4 rounded-xl bg-slate-50 border border-slate-200 space-y-3 overflow-hidden">
                            ...
                            <div className="flex gap-2">
                              <input 
                                ...
                                className="flex-1 bg-white border border-slate-200 rounded-lg px-3 py-2 text-xs font-bold text-slate-755 outline-none focus:border-red-450"
                                placeholder="Escribí PRODUCTOS..."
                              />
                              <button 
                                onClick={handleResetCatalog}
                                disabled={isResettingCatalog || catalogConfirmWord.toUpperCase() !== 'PRODUCTOS'}
                                className="px-4 py-2 bg-red-500 hover:bg-red-600 disabled:bg-slate-200 disabled:text-slate-400 disabled:cursor-not-allowed text-white rounded-lg text-xs font-bold uppercase tracking-wider active:scale-95 transition-all cursor-pointer"
                              >
                                {isResettingCatalog ? '...' : 'Confirmar'}
                              </button>
                              <button onClick={() => { setShowResetCatalog(false); setCatalogConfirmWord(''); }} className="px-3 py-2 border border-slate-200 bg-white text-slate-500 rounded-lg text-xs font-bold cursor-pointer">Cancelar</button>
                            </div>
                          </motion.div>
                        )}
                      </AnimatePresence>
        ...
        {showCreateModal && (
          ...
          >
            <motion.div 
              ...
              className="bg-white rounded-2xl w-full max-w-md overflow-hidden shadow-2xl border border-slate-200 flex flex-col p-6 space-y-4"
            >
              <div className="flex items-center justify-between border-b border-slate-100 pb-3">
                <h3 className="font-bold text-slate-800 flex items-center gap-2">
                  <UserPlus className="w-5 h-5 text-indigo-500" /> Crear Cajero / Usuario
                </h3>
                <button onClick={() => setShowCreateModal(false)} className="p-1.5 hover:bg-slate-100 rounded-lg text-slate-400"><X className="w-5 h-5" /></button>
              </div>
              ...
              <form onSubmit={handleCreateUser} className="space-y-4">
                <div>
                  ...
                  <input 
                    ...
                    className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-xs font-bold text-slate-700 outline-none focus:bg-white focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition-all shadow-inner" 
                ...
                <div>
                  ...
                  <div className="relative">
                    <input 
                      ...
                      className="w-full bg-slate-50 border border-slate-200 rounded-xl pl-4 pr-10 py-2.5 text-xs font-bold text-slate-700 outline-none focus:bg-white focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition-all shadow-inner" 
                ...
                <div>
                  <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">Rol / Permisos</label>
                  <select 
                    value={newRole} 
                    onChange={(e) => setNewRole(e.target.value)} 
                    className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-xs font-bold text-slate-750 outline-none focus:bg-white focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition-all shadow-inner cursor-pointer"
                  >
                ...
                <div className="flex gap-3 pt-2">
                  ...
                  <button type="button" onClick={() => setShowCreateModal(false)} className="px-5 py-3.5 rounded-xl border border-slate-200 text-slate-500 hover:bg-slate-50 font-bold text-xs uppercase tracking-wider active:scale-95 transition-all cursor-pointer">
                    Cancelar
                  </button>
                </div>
              </form>
          ...
          >
            ...
            >
              <div className="flex items-center justify-between border-b border-slate-100 pb-3">
                ...
                <button onClick={() => setShowEditModal(false)} className="p-1.5 hover:bg-slate-100 rounded-lg text-slate-400"><X className="w-5 h-5" /></button>
              </div>
              ...
              <form onSubmit={handleEditUser} className="space-y-4">
                ...
                <div>
                  <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">Nueva Contraseña (Dejar vacío para no cambiar)</label>
                  <div className="relative">
                    <input 
                      type="password" 
                      value={editPassword} 
                ...
                <div className="flex items-center justify-between p-2.5 bg-slate-50 border border-slate-200 rounded-xl shadow-inner">
                  <span className="text-[10px] font-bold text-slate-500 uppercase">Usuario Activo</span>
                  <input 
                    type="checkbox" 
                    checked={editIsActive} 
                    onChange={(e) => setEditIsActive(e.target.checked)} 
                    className="rounded-md border-slate-200 text-indigo-650 focus:ring-indigo-500 h-4.5 w-4.5"
                  />
                </div>
                ...
                <div className="flex gap-3 pt-2">
                  ...
                  <button type="button" onClick={() => setShowEditModal(false)} className="px-5 py-3.5 rounded-xl border border-slate-200 text-slate-500 hover:bg-slate-50 font-bold text-xs uppercase tracking-wider active:scale-95 transition-all cursor-pointer">
                    Cancelar
                  </button>
                </div>
              </form>
            ...
            >
              ...
              <div className="space-y-3">
                ...
                <input 
                  ...
                  className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-xs font-bold text-slate-700 outline-none focus:bg-white focus:border-amber-500 focus:ring-1 focus:ring-amber-500 transition-all shadow-inner"
              ...
              <div className="flex gap-3 pt-2">
                <button 
                  ...
                  className="flex-1 bg-amber-500 hover:bg-amber-600 disabled:bg-slate-200 disabled:text-slate-400 disabled:cursor-not-allowed text-white font-extrabold py-3.5 rounded-xl text-xs uppercase tracking-wider flex items-center justify-center gap-1.5 shadow-md active:scale-95 transition-all"
                >
                ...
                <button 
                  ...
                  disabled={isRestoring}
                  onClick={() => { setShowRestoreModal(false); setSelectedBackupToRestore(null); setUploadedFileToRestore(null); setRestoreConfirmWord(''); }} 
                  className="px-5 py-3.5 rounded-xl border border-slate-200 text-slate-500 hover:bg-slate-50 font-bold text-xs uppercase tracking-wider active:scale-95 transition-all cursor-pointer"
                >
                  Cancelar
                </button>
              </div>
